...
#define BEGIN_COM   cs_low(); esp_rom_delay_us(1)
#define END_COM     esp_rom_delay_us(1); cs_high()
...
PMW3360::PMW3360(spi_host_device_t spi_host, gpio_num_t cs)
    : spi_host(spi_host), cs(cs) {} // fuckaahh syntax 


inline void PMW3360::cs_low() {
    gpio_set_level(cs, 0);
    esp_rom_delay_us(1);
}

inline void PMW3360::cs_high() {
    gpio_set_level(cs, 1);
    esp_rom_delay_us(1);
}

uint8_t PMW3360::spi_transfer(uint8_t data) {
    spi_transaction_t t = {};
    t.length = 8;
    t.tx_buffer = &data;
    t.rx_buffer = &data;
...
## Arguments:
`cs_pin`: The `gpio_num_t` of the CS pin
`CPI`: initial CPI (optional)
*/
esp_err_t PMW3360::begin(unsigned int CPI) {
    inBurst = false;
    ...
    // hard reset
    cs_high(); cs_low(); cs_high(); // ensure that the serial port is reset
    ...
    if (check_signature()) { return ESP_OK; }
    else { return ESP_FAIL; }
}

// public
/* 
setCPI: set sensor CPI

## Arguments:
`cpi`: CPI value
*/
void PMW3360::setCPI(unsigned int cpi) {
    int cpival = (cpi/100)-1; // limits to 0--119 
    if (cpival < 0) {
        cpival = 0;
    }
    else if (cpival > 0x77)
...
// public
/* 
readBurst: get motion data for one frame

## Retruns:
`PWM3360_DATA` motion data struct
*/
PMW3360_DATA PMW3360::readBurst() {    
    unsigned long fromLast = esp_timer_get_time() - lastBurst;
    uint8_t burstBuffer[12] = {};
    
    if(!inBurst || fromLast > 500*1000) {
    ...
    esp_rom_delay_us(35); // waits for tSRAD    

    for (int i = 0; i < 12; i++){ // read burst buffer
        burstBuffer[i] = spi_transfer(0x00);
    } 

    esp_rom_delay_us(1); // tSCLK-NCS for read operation is 120ns
...
// public
/* 
readReg: get one `uint8_t` value from the given `reg_addr`.
...
// public
/* 
writeReg: write one uint8_t value to the given reg_addr.
...
/* 
adns_read_reg: write one uint8_t value to the given reg_addr.
*/
uint8_t PMW3360::adns_read_reg(uint8_t reg_addr) {
    ...
    esp_rom_delay_us(100); // tSRAD is 25, but 100us seems to be stable.
    // read data
    uint8_t data = spi_transfer(0);

    END_COM;
    esp_rom_delay_us(19); //    tSRW/tSRR (=20us) minus tSCLK-NCS

    return data;
}

/* 
adns_write_reg: write one uint8_t value to the given reg_addr
*/
void PMW3360::adns_write_reg(uint8_t reg_addr, uint8_t data) {
    ...
    esp_rom_delay_us(20); // tSCLK-NCS for write operation
    END_COM;
    esp_rom_delay_us(100); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}
...
void PMW3360::adns_upload_firmware() {
    ...
    vTaskDelay(10); // assume that the frame rate is as low as 100fps... even if it should never be that low
    ...
    BEGIN_COM;
    spi_transfer(REG_SROM_Load_Burst | 0x80); // write burst destination adress
    esp_rom_delay_us(15);
...
/* 
Check whether SROM is successfully loaded
## returns:
`bool` whether the rom is loaded correctly
*/
